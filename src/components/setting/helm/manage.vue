<template>
  <div v-loading="loading"
    ...
    <!--helm-create-dialog-->
    <el-dialog title='添加 HELM Chart 仓库'
      ...
      <el-form ref="helm"
        ...
        <el-form-item label="URL"
          ...
          <el-input size="small"
                    ...
                    v-model="helm.url"></el-input>
        </el-form-item>
        <el-form-item label="用户名"
                      prop="username">
          <el-input size="small"
                    v-model="helm.username"></el-input>
        </el-form-item>
        <el-form-item label="密码"
                      prop="password">
          <el-input size="small"
                    type="passsword"
                    v-model="helm.password"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer"
           class="dialog-footer">
        <el-button size="small"
                   @click="dialogHelmCreateFormVisible = false">取 消</el-button>
        <el-button :plain="true"
                   type="success"
                   size="small"
                   @click="repoOperation('add')">保存</el-button>
      </div>
    </el-dialog>
    <!--helm-create-dialog-->

    <!--helm-edit-dialog-->
    <el-dialog title='编辑 HELM Chart 仓库'
               :visible.sync="dialogHelmEditFormVisible"
               :close-on-click-modal="false"
               custom-class="dialog-style"
               width="35%">
      <el-form ref="swapHelm"
               :rules="rules"
               label-width="80px"
               tab-position="left"
               :model="swapHelm">
        <el-form-item label="URL"
          ...
          <el-input size="small"
                    placeholder="http(s)://example.com"
                    v-model="swapHelm.url"></el-input>
        </el-form-item>
        <el-form-item label="用户名"
                      prop="username">
          <el-input size="small"
                    v-model="swapHelm.username"></el-input>
        </el-form-item>
        <el-form-item label="密码"
                      prop="password">
          <el-input size="small"
                    type="passsword"
                    v-model="swapHelm.password"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer"
           class="dialog-footer">
        <el-button size="small"
                   @click="dialogHelmEditFormVisible = false">取 消</el-button>
        <el-button :plain="true"
                   type="success"
                   size="small"
                   @click="repoOperation('update')">保存</el-button>
      </div>
    </el-dialog>
    <!--helm-edit-dialog-->
    <div class="section">
      <el-alert type="info"
                :closable="false">
        <template>
          支持 Harbor Helm Chart 仓库管理，用于存放 push 的 chart<br />
          配置使用可参考
      ...
      <div class="sync-container">
        <el-button :plain="true"
                   @click="dialogHelmCreateFormVisible=true"
                   size="small"
                   type="success">新建</el-button>
      </div>
      <div class="helm-list">
        <template>
          <el-table :data="allHelmRepos"
                    style="width: 100%;">
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                <el-button @click="repoOperation('edit',scope.row)"
                           size="mini">编辑</el-button>
                <el-button @click="repoOperation('delete',scope.row)"
                           size="mini"
...
import { getHelmRepoAPI, createHelmAPI, updateHelmAPI, deleteHelmAPI } from '@api'
